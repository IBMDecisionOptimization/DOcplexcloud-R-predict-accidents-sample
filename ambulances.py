...
NamedPoint = namedtuple('NamedPoint', ['LONGITUDE', 'LATITUDE'])
...
with open("params.csv", 'r') as csvfile:
    read = [rec for rec in csv.reader(csvfile, delimiter=',')]

nb_ambulances = int(read[1][0])

accidents =  pd.read_csv("predicted-accidents.csv", engine='c', header=0, skipinitialspace=True)


locations_prob = {NamedPoint(float(t.LONGITUDE), float(t.LATITUDE)) : float(t.accident_prob) for t in accidents.itertuples(index=False)}
